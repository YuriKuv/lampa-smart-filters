(function() {
    ...
    if (window.SmartFiltersComplete) return;
    window.SmartFiltersComplete = true;
    ...
    // --- Получение параметров из панели фильтрации ---
    function getCurrentFilterParams() {
        try {
            ...
            // Язык (страна)
            var langEl = $('.selectbox-item:contains("Язык оригинала") .selectbox-item__subtitle');
            if (langEl.length && langEl.text() !== 'Не выбрано') {
                params.language = langEl.text();
            }
    ...
    // --- Применение фильтра (открытие раздела с результатами) ---
    function applyFilter(filter) {
        ...
        if (params.type === 'Мультфильмы') mediaType = 'movie';
        if (params.type === 'Аниме') mediaType = 'movie';
        ...
        // Добавляем язык (для мультфильмов и аниме)
        if (params.type === 'Мультфильмы') {
            apiUrl += '&with_genres=16';
        }
        if (params.type === 'Аниме') {
            apiUrl += '&with_genres=16&with_original_language=ja';
        }
        ...
        // Создаём уникальный компонент для этого фильтра
        ...
        // Создаём компонент (как в примерах плагинов)
        var FilterComponent = function(object) {
            ...
            comp.create = function() {
                Lampa.Api.request(apiUrl, function(data) {
                    ...
                    this.empty();
                    Lampa.Noty.show('Ошибка загрузки', 2000);
                }.bind(this));
            };
    ...
    // --- Добавление основного пункта "Мои фильтры" ---
    ...
    // --- Кнопка сохранения в панели фильтрации ---
